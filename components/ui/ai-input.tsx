...
export function MorphPanel() {
  ...
  return (
    <motion.div
      ...
      data-panel
      className={cx(
        "bg-white dark:bg-gray-800 fixed bottom-6 right-6 z-[100] flex flex-col items-center overflow-hidden border dark:border-gray-700 shadow-2xl"
      )}
      initial={false}
      animate={{
...
function DockBar() {
  ...
  return (
    <footer className="mt-auto flex h-[44px] items-center justify-center whitespace-nowrap select-none w-full px-1">
      <div className="flex w-full items-center justify-center gap-2 px-2 h-full">
        ...
        <Button
          type="button"
          className="flex h-fit flex-1 justify-center rounded-full px-2 !py-1"
          variant="ghost"
          onClick={triggerOpen}
        >
          <span className="truncate text-sm font-medium">Ask AI</span>
        </Button>
      </div>
    </footer>
  )
...
function InputForm({ ref, onSuccess }: { ref: React.Ref<HTMLTextAreaElement>; onSuccess: () => void }) {
  ...
  return (
    <form
      ...
      className="absolute bottom-0 right-0 bg-white dark:bg-neutral-900"
          ...
          >
            <div className="flex justify-between py-2 border-b dark:border-neutral-800">
              ...
              <button
                type="submit"
                ref={btnRef}
                className="text-foreground right-4 mt-1 flex -translate-y-[3px] cursor-pointer items-center justify-center gap-1 rounded-[12px] bg-transparent pr-1 text-center select-none hover:opacity-70 transition-opacity"
              >
            ...
            <textarea
              ...
              name="message"
              className="h-full w-full resize-none scroll-py-2 rounded-md p-4 outline-0 bg-transparent text-sm"
              required
...
function KeyHint({ children, className }: { children: string; className?: string }) {
  return (
    <kbd
      className={cx(
        "text-foreground flex h-5 w-fit items-center justify-center rounded-sm border dark:border-neutral-700 px-[6px] font-sans text-xs bg-neutral-100 dark:bg-neutral-800",
        className
      )}
    >
      {children}
